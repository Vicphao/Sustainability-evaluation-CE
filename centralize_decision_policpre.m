...
eii=w(i,m1+m2+1:m1+m2+p)*z1(:,i)+w(i,m1+m2+p+1:m1+m2+p+q)*z2(:,i)+w(i,m1+m2+p+q+1:m1+m2+p+q+s1)*y1(:,i)+w(i,m1+m2+p+q++s1+1:m1+m2+p+q+s1+s2)*y2(:,i);
 ...
 f=-[zeros(1,m1) zeros(1,m2) zeros(1,p) zeros(1,q) zeros(1,s1) zeros(1,s2) aa bb];
 A=[zeros(p,m1) zeros(p,m2) diag(-z1(:,k)') zeros(p,q) zeros(p,s1) zeros(p,s2) ones(p,1) zeros(p,1);
    zeros(s1,m1) zeros(s1,m2) zeros(s1,p) zeros(s1,q) diag(-y1(:,k)') zeros(s1,s2) ones(s1,1) zeros(s1,1);
    zeros(q,m1) zeros(q,m2) zeros(q,p) diag(-z2(:,k)') zeros(q,s1) zeros(q,s2) zeros(q,1) ones(q,1);
    zeros(s2,m1) zeros(s2,m2) zeros(s2,p) zeros(s2,q) zeros(s2,s1) diag(-y2(:,k)') zeros(s2,1) ones(s2,1);
    (eps-1)*x1(:,i)' eps*x2(:,i)' eps*z1(:,i)' (eps-1)*z2(:,i)' zeros(1,s1) zeros(1,s2) 0 0;
    eps*x1(:,i)' (eps-1)*x2(:,i)' (eps-1)*z1(:,i)' eps*z2(:,i)' zeros(1,s1) zeros(1,s2) 0 0;
    -x1' zeros(n,m2) z1' -z2' y1' zeros(n,s2) zeros(n,1) zeros(n,1);
    zeros(n,m1) -x2' -z1' z2' zeros(n,s1) y2' zeros(n,1) zeros(n,1)];
 b=zeros(2*n+p+q+s1+s2+2,1);
 Aeq=[x1(:,i)' x2(:,i)' z1(:,i)' z2(:,i)' zeros(1,s1) zeros(1,s2) 0 0;
     -eii*x1(:,i)' -eii*x2(:,i)' (1-eii)*z1(:,i)' (1-eii)*z2(:,i)' y1(:,i)' y2(:,i)' 0 0];
 beq=[1;0];
 lb=epsilon.*ones(1,m1+m2+p+q+s1+s2+2); 
 ub=[];
