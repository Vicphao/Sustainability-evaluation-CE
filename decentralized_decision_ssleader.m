...
for i=1:n
    ...
    Aeq=[x1(:,i)' zeros(1,m2) zeros(1,p) z2(:,i)' zeros(1,s1) zeros(1,s2);
        zeros(1,m1) t1(i,:)*x2(:,i)' t1(i,:)*z1(:,i)' -z2(:,i)'  zeros(1,s1) -y2(:,i)'];
    beq=[1;0];
    lb=epsilon.*ones(1,m1+m2+p+q+s1+s2); 
    ub=[];
    [w(i,:),fval(:,i)]=linprog(f,A,b,Aeq,beq,lb,ub); 
    eii=w(i,m1+m2+1:m1+m2+p)*z1(:,i)+w(i,m1+m2+p+q+1:m1+m2+p+q+s1)*y1(:,i);
    for k=1:n % ss leader
    f=-[zeros(1,m1) zeros(1,m2)  zeros(1,p)  zeros(1,q) zeros(1,s1)  zeros(1,s2) 1];
    A=[ zeros(s1,m1) zeros(s1,m2) zeros(s1,p) zeros(s1,q) diag(-y1(:,k)') zeros(s1,s2) ones(s1,1);
        zeros(p,m1) zeros(p,m2) diag(-z1(:,k)') zeros(p,q) zeros(p,s1) zeros(p,s2) ones(p,1);
    ...
    b=zeros(2*n+s1+p+2,1);
    %zeros(p,m1) zeros(p,m2) diag(-z1(:,k)') zeros(p,q) zeros(p,s1) zeros(p,s2) ones(p,1);
    Aeq=[-eii*x1(:,i)' zeros(1,m2)  z1(:,i)' -eii*z2(:,i)' y1(:,i)' zeros(1,s2)  0;
         x1(:,i)' zeros(1,m2) zeros(1,p) z2(:,i)' zeros(1,s1) zeros(1,s2) 0;
         zeros(1,m1) t1(i,:)*x2(:,i)'  t1(i,:)*z1(:,i)' -z2(:,i)' zeros(1,s1) -y2(:,i)' 0];
    beq=[0;1;0];
